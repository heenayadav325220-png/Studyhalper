...
export function HomeworkSolver({ user, language, isTagMode }: Props) {
  ...
  const handleSolve = async () => {
    ...
    try {
      ...
      setAnswer(translate('solver_error', language, "Sorry, I couldn't solve that right now."));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={`p-6 rounded-3xl border shadow-sm space-y-4 ${isTagMode ? 'bg-slate-950 border-cyan-500/50' : 'bg-white border-slate-150/70'}`}>
      <h3 className={`text-xl font-bold ${isTagMode ? 'text-cyan-400' : 'text-slate-800'}`}>
        {translate('homework_solver_title', language, 'Homework Solver')}
      </h3>
      <textarea
        value={prompt}
        onChange={(e) => setPrompt(e.target.value)}
        placeholder={translate('homework_placeholder', language, 'Enter your homework question here...')}
      ...
      >
        {loading ? translate('solving_btn', language, 'Solving...') : translate('solve_btn', language, 'Solve')}
      </button>
      {answer && (
        <div className={`mt-4 p-4 rounded-xl ${isTagMode ? 'bg-cyan-950 text-cyan-200' : 'bg-slate-50 text-slate-700'}`}>
          <h4 className="font-semibold mb-2">{translate('answer_label', language, 'Answer:')}</h4>
