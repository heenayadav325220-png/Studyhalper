...
export function StudyTimer({ isTagMode, onSessionComplete }: StudyTimerProps) {
  ...
  useEffect(() => {
    ...
    if (isActive && seconds > 0) {
      ...
      alert(`🎉 Fantastic work! You've completed a ${minutesCompleted}-minute focus session on ${selectedSubject}! Your study session is logged to your learning history.`);
      setSeconds(25 * 60);
    } else if (!isActive && seconds !== 0) {
  ...
  }, [isActive, seconds, originalDuration, selectedSubject, onSessionComplete]);
  ...
  return (
    <div className={`p-6 rounded-3xl border shadow-sm space-y-4 ${isTagMode ? 'bg-slate-950 border-cyan-500/50 shadow-[0_0_15px_rgba(34,211,238,0.2)]' : 'bg-indigo-50 border-indigo-100'}`}>
      <div className="flex justify-between items-center border-b pb-2 border-indigo-100/30">
        <h3 className={`text-xs font-extrabold uppercase tracking-tight ${isTagMode ? 'text-cyan-400' : 'text-indigo-900'}`}>Focus Timer</h3>
        <span className={`text-2xl font-mono font-bold ${isTagMode ? 'text-cyan-300' : 'text-indigo-700'}`}>{formatTime(seconds)}</span>
      </div>

      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-[8px] uppercase font-black text-slate-400 tracking-wider mb-1">Study Subject</label>
          <select 
        ...
        <div>
          <label className="block text-[8px] uppercase font-black text-slate-400 tracking-wider mb-1">Duration Presets</label>
